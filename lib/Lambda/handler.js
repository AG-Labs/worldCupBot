  ...
  if (event.Records) {
    for (let singleEvent of event.Records) {
      ...
      if (messageBody.delay) {
        ...
        if (messageBody.type === "mid") {
          ...
          const gameResponse = await fetch(gameParameters.url, {
            method: gameParameters.method,
            headers: gameParameters.headers,
          });
          if (!gameResponse.ok) {
            throw new Error(`HTTP error! status: ${gameResponse.status}`);
          }
          const gameResult = await gameResponse.json();
          const gameStatus = gameResult.response[0].fixture.status;

          const gameScore = gameResult.response[0].score;
          const gameGoals = gameResult.response[0].goals;
          ...
          if (gameStatus.short === "HT") {
            ...
            if (personAScore == 0 && personBScore == 0) {
              let randomMessage = Math.floor(
                Math.random() * finalNilnilMiddle.length,
              );
              let messageFunction = finalNilnilMiddle[randomMessage];
              ...
              let randomMessage = Math.floor(
                Math.random() * finalMiddleMessageDraw.length,
              );
              let messageFunction = finalMiddleMessageDraw[randomMessage];
              ...
              let randomMessage = Math.floor(
                Math.random() * finalMiddleMessageWinner.length,
              );
              let messageFunction = finalMiddleMessageWinner[randomMessage];
        ...
        if (messageBody.type === "end") {
          ...
          const gameStatus = gameResult.response[0].fixture.status;
          try {
            console.log(JSON.stringify(gameResult.response[0], null, 2));
          } catch (errorInString) {
            console.error(errorInString);
          }
          console.log(gameResult.response[0]);
          ...
          if (["FT", "AET", "PEN"].includes(gameStatus.short)) {
            ...
            if (personAScore === personBScore) {
              ...
              let randomMessage = Math.floor(
                Math.random() * finalEndMessage.length,
              );
              let messageFunction = finalEndMessage[randomMessage];
    ...
    try {
      ...
      for (let response of resolvedMap) {
        ...
        let randomMessage = Math.floor(
          Math.random() * finalStartingMessages.length,
        );
        let messageFunction = finalStartingMessages[randomMessage];
