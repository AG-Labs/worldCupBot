...
const eurosLeague = 4;
const worldCupLeague = 1;
const season = 2024;
let currentStandings = createRequest(
  `standings?league=${worldCupLeague}&season=${season}`,
);

fetch(currentStandings.url, {
  method: currentStandings.method,
  headers: currentStandings.headers,
})
  .then((response) => {
    if (!response.ok) {
  ...
  .then((gameResult) => {
    let teams = gameResult.response[0].league.standings.flat();
